...
export default function DashboardPage() {
  ...
  const fetchVideos = async (uid: string) => {
    const { data, error } = await supabase
      .from("videos")
      .select("id, user_id, file_path, created_at, archetype, rating_3pt, rating_3pt_observed, rating_finishing, rating_finishing_observed, rating_handles, rating_handles_observed, nba_comparison, comparison_reason, confidence, confidence_note")
      .eq("user_id", uid)
  ...
  const handleConfirmDelete = async () => {
    ...
    // Defense in depth — the dashboard only ever lists the current user's
    // own videos, but never trust that alone. Row Level Security enforces
    // the same user_id check server-side on every delete below.
    if (deleteTarget.user_id !== userId) {
      setDeleteError("You can only delete your own videos.");
      return;
    }
    ...
    // Best-effort — analytics rows for this video aren't critical to the
    // delete succeeding, so a failure here doesn't block the rest.
    await supabase
      .from("events")
      .delete()
      .eq("video_id", deleteTarget.id)
      .eq("user_id", userId);
    ...
    if (storageError) {
      setDeleteError("Could not delete the video file: " + storageError.message);
      setDeleting(false);
      return;
    }
    ...
    const { error: dbError } = await supabase
      ...
      .eq("user_id", userId);
    if (dbError) {
      setDeleteError("Could not delete the video record: " + dbError.message);
      setDeleting(false);
      return;
    }
          ...
          {libraryVideos.length === 0 && videos.length === 0 ? (
            ...
            <ul className="space-y-4">
              {libraryVideos.map((video) => {
                ...
                return (
                  <li key={video.id} className="group rounded-xl border border-white/8 bg-white/3 overflow-hidden">
                    ...
                    <div className="px-5 py-4 flex items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <button
                          onClick={() => { setDeleteTarget(video); setDeleteError(""); }}
                          aria-label="Delete video"
                          className="shrink-0 rounded-md border border-white/10 p-1.5 text-gray-400 hover:border-red-500/40 hover:text-red-400 transition-colors opacity-100 sm:opacity-0 sm:group-hover:opacity-100"
                        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-gray-950 p-8 shadow-2xl">
            ...
            <h2 className="text-xl font-bold mb-2">
              Delete this video and player card?
            </h2>
            <p className="text-sm text-gray-500 mb-6">
              Your{" "}
              {deleteTarget.archetype ? (
                <span className="font-semibold text-white">{deleteTarget.archetype}</span>
              ) : (
                "player"
              )}{" "}
              card and its share link will be permanently deleted. This can&apos;t be undone.
            </p>
            ...
            <div className="flex flex-col gap-2">
              <button
                ...
                disabled={deleting}
                className="w-full rounded-lg bg-red-600 hover:bg-red-500 disabled:opacity-50 transition-colors py-2.5 text-sm font-semibold text-white"
              >
              ...
              <button
                ...
                disabled={deleting}
                className="w-full rounded-lg py-2.5 text-sm text-gray-500 hover:text-gray-400 transition-colors disabled:opacity-50"
              >
                Cancel
              </button>
            </div>
